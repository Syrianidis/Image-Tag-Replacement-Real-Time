function CVrealVideo()

% Define frame rate
NumberFrameDisplayPerSecond=10;
imWrp1=imread('tag4.jpg');
imWrp2=imread('tag5.jpg');
imWrp3=imread('tag6.jpg');
imRep=imread('butterfly.jpg');

imWrp1=imresize(imWrp1,.5);
imWrp2=imresize(imWrp2,.5);
imWrp3=imresize(imWrp3,.5);
...
% set up timer object
TimerData=timer('TimerFcn', {@FrameRateDisplayTest,vid,imWrp1,imWrp2,imWrp3,imRep},'Period',1/NumberFrameDisplayPerSecond,'ExecutionMode','fixedRate','BusyMode','drop');
...
function FrameRateDisplayTest(obj, event,vid, imWrp1,imWrp2,imWrp3,imRep)
persistent IM;
...
%    handlesPlot=BDDoGThres(IM,.4,.9,1/10);
   imOut=imImpose(imresize(IM,.5),imWrp1,imRep,'SURF');
   imOut=imImpose(imOut,imWrp2,imRep,'SURF');
   imImpose(imOut,imWrp3,imRep,'SURF');
   title('Imposing Images on Tags')
% else
